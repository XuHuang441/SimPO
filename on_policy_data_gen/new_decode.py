...
max_prompt_len = args.max_tokens - 1
...
def truncate_prompt(example):
    ...
    if len(prompt_tokens) > max_prompt_len:
        truncation_len = len(prompt_tokens) - max_prompt_len
    ...
    # 打印被截断的 token 数
    if truncation_len > 0:
        print(f"[截断] 原始 token 长度: {len(prompt_tokens) + truncation_len} → 截断 {truncation_len} 个 token")

    return {"prompt": final_prompt}

# tqdm 显示处理进度
train_dataset = train_dataset.map(truncate_prompt, desc="正在处理 prompt",)
...
batched_prompts = list(chunked(prompts, args.batch_size))
output_data = []
